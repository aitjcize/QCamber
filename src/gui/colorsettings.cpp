    ...
    ui(new Ui::ColorSettings)
{
    ...
    for(int i=1;i<COLOR_NUMBER+1;i++)
    {
        ...
        label->setStyleSheet("QLabel { background-color: "+cfg.value("color/"+label_name).toString()+"; color: black; }");
        connect(label, SIGNAL(clicked()),colorSignalMapper, SLOT(map()));
        colorSignalMapper->setMapping(label,label_name);
        label_list.append(label);
        ui->horizontalLayout->addWidget(label);
    }
    connect(colorSignalMapper,SIGNAL(mapped(const QString &)),this,SLOT(openSelector(const QString &)));
...
QColor ColorSettings::openSelector(const QString  color_config)
{
    ...
    label_list.at(color_config.toInt()-1)->setStyleSheet("QLabel { background-color: "+color.name()+"; color: black; }");
    return color;
}
